...
func (kr *kubernetesRestorer) RestoreWithResolvers(
	...
	restoreCtx := &restoreContext{
		...
		useInformerCacheForGet:         req.UseInformerCacheForGet,
	}
	...
	if ctx.useInformerCacheForGet {
		...
		for _, informerResource := range selectedResourceCollection {
			...
			for _, items := range informerResource.selectedItemsByNamespace {
				...
				_, factory, err := ctx.getGenericInformerInternal(gr, items[0].version, items[0].targetNamespace)
				if err != nil {
					warnings.AddVeleroError(errors.Wrapf(err, "Error initializing informer for %v, %v", gr, items[0].targetNamespace))
					continue
				}
				if factory != nil {
...
func (ctx *restoreContext) getGenericInformerInternal(groupResource schema.GroupResource, version, namespace string) (informers.GenericInformer, *informerFactoryWithContext, error) {
	...
	return informer, returnFactory, nil
}

func (ctx *restoreContext) getGenericInformer(groupResource schema.GroupResource, version, namespace string) (informers.GenericInformer, error) {
	informer, factoryWithContext, err := ctx.getGenericInformerInternal(groupResource, version, namespace)
	if err != nil {
		return nil, err
	}
...
func (ctx *restoreContext) getResourceLister(groupResource schema.GroupResource, obj *unstructured.Unstructured, namespace string) (cache.GenericNamespaceLister, error) {
	informer, err := ctx.getGenericInformer(groupResource, obj.GroupVersionKind().Version, namespace)
	if err != nil {
		return nil, err
	}
	if namespace == "" {
...
func (ctx *restoreContext) restoreItem(obj *unstructured.Unstructured, groupResource schema.GroupResource, namespace string) (results.Result, results.Result, bool) {
	...
	// new namespace
	if ctx.useInformerCacheForGet {
		ctx.log.Infof("Checking for existence %s: %v", obj.GroupVersionKind().Kind, name)
	...
	if err != nil || fromCluster == nil {
		...
		ctx.log.Infof("Creating %s: %v", obj.GroupVersionKind().Kind, name)
